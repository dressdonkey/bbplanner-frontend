...
@Component({
  ...
  styleUrls: ['./edit-team-logo.component.css']
})
...
export class EditTeamLogoComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<EditTeamLogoComponent>,
  ...
  onUploadOutput(output: UploadOutput): void {
    ...
    if (output.type === 'allAddedToQueue') { // when all files added in queue
      ...
      if(output.file.responseStatus == 400){
        ...
        this.team.logo = output.file.response.team.logo;  
  ...
  startUpload(id): void {
    ...
    const event: UploadInput = {
      type: 'uploadFile',
      url: 'http://192.168.33.10/api/teams/upload/'+id,
      method: 'POST',
      data: { 
        id: id 
      },
